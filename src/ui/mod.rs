...
pub fn footer(state: &State, theme: Theme, width: usize, hints: &str) -> Line<'static> {
...
mod tests {
    ...
    fn footer_shows_an_open_prompt() {
        ...
        let line = footer(&state, Theme::new(false), 40, "hints");
        assert!(rendered(&line).starts_with(" /ship"));
    }
    ...
    fn footer_prefers_a_status_over_hints() {
        ...
        let line = footer(&state, Theme::new(false), 40, "hints");
        assert!(rendered(&line).contains("Last row"));
        assert!(!rendered(&line).contains("hints"));
    }
